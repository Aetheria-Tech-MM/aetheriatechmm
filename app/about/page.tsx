  ...
  return (
    <main id="main">
      ...
      <section className="py-20 sm:py-28"><div className="site-container grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
        ...
        <div className="relative min-h-80 rounded-lg border border-border bg-muted/30 p-6" aria-hidden="true"><div className="w-2/5 rounded-md border border-border bg-background p-5"><span className="block h-2 w-16 rounded bg-brand" /><span className="mt-4 block h-2 w-full rounded bg-foreground/10" /></div><div className="absolute right-6 top-16 w-1/2 rounded-lg border border-brand/50 bg-background p-6 shadow-[var(--shadow-md)]"><span className="block h-2 w-4/5 rounded bg-foreground/15" /><span className="mt-4 block h-8 w-20 rounded bg-brand-soft" /><span className="mt-4 block h-2 w-full rounded bg-foreground/10" /></div><div className="absolute bottom-6 left-16 h-20 w-2/5 rounded-md border border-dashed border-border" /></div>
      </div></section>

      <section className="border-y border-border bg-surface-sunken py-20"><div className="site-container grid gap-10 md:grid-cols-2"><div><p className="eyebrow">Mission</p><h2 className="mt-3 font-heading text-3xl font-bold leading-tight">Build practical technology that makes businesses simpler and more efficient.</h2></div><div><p className="eyebrow">Vision</p><h2 className="mt-3 font-heading text-3xl font-bold leading-tight">Create software products that become trusted, everyday tools.</h2></div></div></section>
      ...
      <section className="border-y border-border bg-surface-sunken py-20 sm:py-26"><div className="site-container"><SectionHeading eyebrow="Engineering" title="Built to be dependable">We keep this part deliberately unglamorous: dependable software, not a showcase of tools.</SectionHeading><div className="grid gap-x-10 gap-y-8 md:grid-cols-2 lg:grid-cols-3">{principles.map(([title, text], index) => <div key={title} className="border-t border-border pt-5"><span className="text-xs font-bold text-brand">0{index + 1}</span><h3 className="mt-4 font-heading text-lg font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p></div>)}</div></div></section>

      <section className="border-y border-onyx-border bg-onyx py-20 text-onyx-foreground sm:py-26"><div className="site-container max-w-4xl"><p className="eyebrow">Looking ahead</p><h2 className="mt-4 font-heading text-4xl font-bold tracking-tight text-onyx-foreground sm:text-5xl">Building toward a portfolio, one product at a time.</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-onyx-muted">FOM is where we&apos;re starting. We&apos;re building toward practical software products, each designed around a real business need.</p><Link href="/contact" className={cn(buttonVariants({ variant: "outline", size: "lg" }), "mt-8 border-onyx-border bg-transparent px-5 text-onyx-foreground hover:border-onyx-foreground hover:bg-[#1f2444] hover:text-onyx-foreground")}>Talk to us</Link></div></section>
    </main>
  )
}
