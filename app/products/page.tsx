...
export default function ProductsPage() {
  return (
    <main id="main">
      ...
      <section className="py-20 sm:py-28"><div className="site-container grid gap-12 lg:grid-cols-2 lg:items-center"><div><SectionHeading title="Your customers message you. FOM turns it into an order." /><div className="space-y-4 leading-7 text-muted-foreground"><p>Orders, customers, reports, and delivery are kept in one workspace, synced across web, Android, and iOS. A team selects the part of the conversation that matters, and FOM prefills the draft order for review.</p><p>FOM is built specifically for chat-first shop operations — not a general-purpose order tool retrofitted for chat.</p></div></div><div className="rounded-3xl border border-border bg-muted/30 p-6"><div className="max-w-xs rounded-2xl border border-border bg-background p-4"><div className="h-8 rounded-full bg-muted" /><div className="ml-auto mt-4 h-12 w-4/5 rounded-2xl bg-primary/15" /><div className="mt-4 h-8 w-3/5 rounded-full bg-muted" /></div><div className="relative -mt-8 ml-auto w-3/5 rounded-2xl border border-primary/50 bg-background p-5 shadow-xl"><p className="text-xs font-bold text-primary">DRAFT ORDER</p><div className="mt-5 space-y-3">{["Customer", "Items", "Delivery"].map((item) => <div key={item}><p className="text-[10px] text-muted-foreground">{item}</p><div className="mt-1 h-2 rounded bg-foreground/12" /></div>)}</div><div className="mt-6 h-9 rounded-lg bg-foreground" /></div></div></div></section>
      ...
      ].map(([title, text], index) => <li key={title}><Card className="h-full"><CardContent><span className="text-sm font-bold text-primary">0{index + 1}</span><h3 className="mt-8 font-heading text-lg font-bold">{title}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{text}</p></CardContent></Card></li>)}</ol></div></section>
      ...
      <section id="pricing" className="bg-muted/30 py-20 sm:py-28"><div className="site-container"><SectionHeading eyebrow="Pricing" title="Simple, transparent pricing">One subscription per shop. See getfom.com for current pricing and active promotions.</SectionHeading><Card className="max-w-xl"><CardContent className="p-8"><p className="text-sm text-muted-foreground">Shop Monthly</p><p className="mt-3 font-heading text-4xl font-extrabold">88,000 MMK <span className="font-sans text-base font-normal text-muted-foreground">/ month</span></p><ul className="my-8 space-y-3 text-sm">{["1 shop", "1 connected channel", "Up to 3 staff accounts", "All core features", "7-day free trial"].map((item) => <li key={item} className="flex items-center gap-3"><span className="grid size-5 place-items-center rounded-full bg-primary/15 text-xs font-bold text-primary">✓</span>{item}</li>)}</ul><Link href="https://getfom.com/register" className={cn(buttonVariants({ size: "lg" }), "w-full")}>Start free trial</Link><p className="mt-4 text-center text-xs text-muted-foreground">No credit card required · Add-ons available · Yearly billing saves 17%</p></CardContent></Card></div></section>

      <section className="py-16"><div className="site-container"><div className="flex flex-col items-start justify-between gap-6 rounded-2xl border border-border p-7 sm:flex-row sm:items-center"><div><h2 className="font-heading text-xl font-bold">Ready to see FOM in your shop?</h2><p className="mt-2 text-muted-foreground">Start a 7-day free trial — no credit card required.</p></div><Link href="https://getfom.com/" className={cn(buttonVariants({ size: "lg" }), "px-5")}>Explore FOM</Link></div></div></section>
    </main>
  )
}
